...
export function LLMTurnsModal({
  ...
  const turnLines = useMemo(() => {
    return turns.map((turn, index) => {
      ...
      // 为列表渲染提供适当截断的预览文本，防止数十万字符的单行直接撑爆 DOM 排版引擎
                ...
                >
                  {filteredTurnLines.map((item) => (
                    ...
                    >
                      ...
                      <span
                        className="flex-1 select-text text-ink group-hover:text-accent font-mono truncate max-w-5xl"
                        title={item.preview}
                      >
                        {wrapLines ? item.preview : item.preview}
                      </span>
