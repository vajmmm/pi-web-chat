...
export function createCoordinatorExtension(
  ...
  return {
    ...
    factory: (pi: ExtensionAPI) => {
      // 0. 按需查询可用角色列表工具 (list_available_roles)
      pi.registerTool(
        defineTool({
          name: "list_available_roles",
          label: "查询可用子角色",
          description:
            "按需查询系统当前所有可用的子智能体角色列表、职责定位、严格禁令、能力范围与已授权工具；它是角色发现工具，不是调用 spawn_subagent 的强制前置步骤。",
      ...
      pi.registerTool(
        defineTool({
          ...
          label: "派发子任务",
          description:
            "创建一个带 Task Contract 的异步 Subagent 任务。Developer/Verifier 用于独立、可验收的工程工作；canonical researcher 也支持小而明确、自包含的只读 factual probe，目标是快速返回事实、证据和出处，不要求代码实现闭环。Runtime 管理 workspace、lifecycle、verification 与结果回传；完成后会自动回传结果，不要轮询或探测状态。",
          promptSnippet: "按角色派发异步 Subagent：工程工作或只读 Researcher probe",
          promptGuidelines: [
            "Developer/Verifier：为独立、可验收的工程工作填写清晰的 goal、scope、context_files 和 acceptance_criteria；互不依赖的任务可以并行派发。",
            "Researcher：允许派发小而明确、自包含的只读 probe；用 goal、context_files 和 acceptance_criteria 描述要核实的事实、证据与出处，不要求代码实现闭环。对能减少 Coordinator Context 消耗或带来有价值并行的读取，倾向尽早派发。",
          ],
          executionMode: "parallel",
          parameters: Type.Object({
            role: Type.String({
              description: "子智能体角色标识。使用 canonical role，例如 'developer'、'verifier'、'researcher'；list_available_roles 仅在需要发现或比较角色时按需调用，不是 spawn researcher 的前置步骤。严禁使用 default 或 coordinator",
            }),
          ...
          async execute(_toolCallId, params) {
            ...
            if (!validRoles.includes(params.role)) {
              return {
                ...
                content: [
                  {
                    type: "text",
                    text: `[spawn_subagent] 失败: 非法的子智能体角色 '${params.role}'。当前仅支持 canonical 子智能体角色：developer、verifier、researcher。list_available_roles 仅用于按需发现角色。`,
                  },
                ],
              };
            }
            ...
            try {
              const contractDefaults = getDefaultTaskContractFields(params.role as AgentRole);
              const taskContract: TaskContract = {
                ...
                expectedEffects: params.expected_effects ?? contractDefaults.expectedEffects,
                contextFiles: params.context_files,
                scope: {
                  include: params.scope_include ?? contractDefaults.scope?.include,
                  exclude: params.scope_exclude ?? contractDefaults.scope?.exclude,
                },
                acceptanceCriteria: params.acceptance_criteria ?? contractDefaults.acceptanceCriteria,
      ...
      pi.registerTool(
        defineTool({
          ...
          label: "复用智能体执行新任务",
          description:
            "复用处于 idle_reusable 状态的 Subagent 知识执行一个新的 Task Contract。该调用异步运行，Runtime 会自动回传结果；不要轮询或探测状态。仅当本任务明确修复/重做某个历史任务时填写 rework_of_task_id，否则视为独立新任务。",
          promptSnippet: "复用 idle_reusable Agent 执行新 Task Contract",
          parameters: Type.Object({
          ...
          async execute(_toolCallId, params) {
            ...
            try {
              ...
              const contractDefaults = getDefaultTaskContractFields(targetRole);
              const taskContract: TaskContract = {
                taskId: "",
                parentSessionId: ctx.parentSessionId,
                role: targetRole,
                    ...
                    text: JSON.stringify(
                      {
                        ...
                        branch: task.branchName,
                        worktree: task.worktreePath ?? null,
                        message: `已复用 Agent [${task.agentId}] 启动异步新任务 [${task.taskId}]。执行完毕后成果将由系统自动打断当前会话并直接注入主会话，请直接结束当前发言等待结果注入。`,
                      },
                      null,
                      2,
                    ),
      ...
      pi.on("before_agent_start", async (event, ctx) => {
        ...
        const capabilities = sessionCtx.getMainSessionCapabilities?.() ?? {
          productDesign: false,
          imageInput: false,
